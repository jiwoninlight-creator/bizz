'use client'

import Link from 'next/link'
import { createClient } from '@/lib/supabase-client'

function GoogleIcon() {
  return (
    <svg className="h-5 w-5 shrink-0" viewBox="0 0 24 24" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  )
}

export default function LoginPage() {
  const handleGoogleLogin = async () => {
    const supabase = createClient()
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback`,
      },
    })
  }

  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-zinc-950">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-1/3 top-24 h-80 w-80 rounded-full bg-indigo-600/20 blur-[100px]" />
        <div className="absolute bottom-1/4 right-1/4 h-72 w-72 rounded-full bg-violet-600/15 blur-[100px]" />
      </div>

      <div className="relative z-10 px-6 pt-6">
        <Link
          href="/"
          className="inline-flex items-center gap-1 text-sm text-zinc-500 transition-colors hover:text-zinc-300"
        >
          ← 처음으로
        </Link>
      </div>

      <div className="relative z-10 mx-auto flex w-full max-w-xs flex-1 flex-col items-center justify-center px-6 pb-16">
        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white">
          <span className="text-2xl font-bold text-zinc-900">B</span>
        </div>
        <h1 className="mt-4 text-2xl font-bold text-white">BIZZ</h1>
        <p className="mt-1 text-sm text-zinc-400">
          학교 생활의 모든 순간을, 한 곳에
        </p>

        <button
          type="button"
          onClick={handleGoogleLogin}
          className="mt-10 flex w-full max-w-xs items-center justify-center gap-3 rounded-xl bg-white px-6 py-3.5 font-medium text-zinc-900 transition-colors hover:bg-zinc-100"
        >
          <GoogleIcon />
          Google로 계속하기
        </button>

        <p className="mt-6 text-center text-xs text-zinc-500">
          학교에서 발급받은 구글 계정으로만 가입할 수 있어요
        </p>
      </div>
    </div>
  )
}
